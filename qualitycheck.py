...
if __name__ == "__main__":
    ...
    T, errors, i, total_time = icp.icp(data, ref, init_pose=None, max_iterations=100, tolerance=0.001)
    print('\nTransformation found by ICP:\n', T)
    print('\nicp time :', total_time, 's')
    print('\nicp residual error :', errors[i-1], 'mm')
    ...
    homogenous_model = np.ones((data.shape[0], 4))   ##### Construct a [N,4] matrix
    homogenous_model[:,0:3] = np.copy(data)                   ##### Replace the X
    ...
    transformed_model = np.dot(T, homogenous_model.T).T
    ...
    datatools.draw_data_and_ref(transformed_model, ref=ref, title='Registered data', ax=ax)
    
    #### Display error progress over time
    fig1 = plt.figure(2, figsize=(20,3))
    it = np.arange(0,len(errors),1)
    plt.plot(it, errors[0:len(errors)], 'b-')
    ...
    # np.savetxt('data02_object.xyz',transformed_model, delimiter=' ')
    np.savetxt('data/data03.xyz',transformed_model, delimiter=' ')